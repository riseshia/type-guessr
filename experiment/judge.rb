...
#   ruby judge.rb <results_dir> --task t1_call_chain # score one task only
...
JUDGE_MODEL = ENV.fetch("JUDGE_MODEL", "opus")
...
FILENAME_PATTERN = /\A(.+)_(BASE|LSP_GUIDED|LSP|TG_NATURAL|TG_GUIDED)_(t\d+)\z/
...
def print_summary(scores)
  ...
  conditions = %w[BASE LSP LSP_GUIDED TG_NATURAL TG_GUIDED]
  ...
  tasks.each do |task|
    puts "\n  #{task}:"
    puts format("    %-12s %8s %8s %8s %8s %3s", "Cond", "Compl", "Acc", "Depth", "Overall", "N")
    puts "    " + "-" * 51
  ...
  conditions.each do |cond|
    ...
    puts format("    %-12s %4.1f +/- %4.1f %4.1f +/- %4.1f %4.1f +/- %4.1f %4.1f +/- %4.1f %3d",
      cond,
