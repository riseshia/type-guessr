...
module TypeGuessr
  module CLI
    ...
    module Analyzer
      ...
      # Categories: never (zero duck-typing candidates) / direct_missing (typed,
      # but the type cannot answer a call) / unknown (no judgment) / duck_typed / direct.
      ...
      def self.dump_coverage_site(node, result, file_path, lines)
        ...
        category =
          if result.type.is_a?(Core::Types::Never)
            result.reason.to_s.match?(/does not (define|accept)/) ? "direct_missing" : "never"
