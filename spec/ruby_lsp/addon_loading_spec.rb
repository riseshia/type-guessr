...
RSpec.describe "Addon Loading" do
  describe "addon dependencies" do
    ...
    it "uses absolute paths for core references in VariableTypeResolver" do
      resolver_file = File.read("lib/ruby_lsp/type_guessr/variable_type_resolver.rb")

      expect(resolver_file).to match(/::TypeGuessr::Core::TypeResolver/)
      expect(resolver_file).to match(/::TypeGuessr::Core::ScopeResolver/)
    end

    it "explicitly requires variable_type_resolver in Hover" do
      hover_file = File.read("lib/ruby_lsp/type_guessr/hover.rb")

      expect(hover_file).to match(/require_relative.*variable_type_resolver/)
    end
  end
end
